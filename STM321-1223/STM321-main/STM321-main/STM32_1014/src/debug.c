...
uint8_t Config_Process(void)
{
	...
	/**********************************************/
	if(strstr((char*)Config.RxBuffer,"RTIME")!=NULL)
	{
		Count=1;
		mem_set(Address,0,10);
		xHour=(Config.RxBuffer[6]-'0')*10+(Config.RxBuffer[7]-'0');
		xMin=(Config.RxBuffer[9]-'0')*10+Config.RxBuffer[10]-'0';
		xSec=(Config.RxBuffer[12]-'0')*10+Config.RxBuffer[13]-'0';
		xDay=(Config.RxBuffer[15]-'0')*10+Config.RxBuffer[16]-'0';
		AddtoRead=Caculator_Add2Write16(xDay,xHour,xMin,xSec);
		FLASH_ReadBuffer((int)AddtoRead,Config.TxBuffer,14);
		ConvertGPS(&DataGPSConverted);
		sprintf(buf,"ReadTime=%d:%d:%d-%d/%d/%d, Lat= %f, Long= %f, Speed %d\n\r",
								DataGPSConverted.GPSTime.hour,DataGPSConverted.GPSTime.min,
								DataGPSConverted.GPSTime.sec,DataGPSConverted.GPSTime.mday,
								DataGPSConverted.GPSTime.month,DataGPSConverted.GPSTime.year,
								(double)DataGPSConverted.Latitude,(double)DataGPSConverted.Longitude,DataGPSConverted.GPSSpeed);
		Sendstring(buf);
	}
