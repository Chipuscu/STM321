...
uint32_t Create(const char * filename)
{
	...
	maxfile =MAXFILE;
	xbuff[4]=1;
	// Tim kiem cac file trong listfile
	...
	// Tim kiem vi tri con trong tren listfile
	for(Add=0,i=0;i<30;i++)
	{
	...
	FLASH_WriteBuffer(xbuff,address,5);  // 1208
	Delay_ms(5);
...
{
	uint8_t i,z,Add,Data8[5],Copy=0,Number=1,count;
	uint16_t Data16[2],Address,xData8[4066];
	...
	for(i=0;i<30;++i)
	{
		...
		if(Data16[0]==filename_hash(filename))
		{
			...
			if(Data8[4]==Number)
			{
				FLASH_ReadBuffer(Address+30,xData8,4066);
				Send3((uint16_t *)xData8,4066);
				Number++;
//				if(length<(MAXFILE-30)) // nam trong 1 sector
//				{
//					FLASH_ReadBuffer(Address+30,xData8,4066);
//					Sendstring((char *)xData8);
////					for(z=0;z<(length/1024);++z)	// so lan doc
////					{
////						if(length<1024)		// neu length nho hon fileread
////						{
////							FLASH_ReadBuffer(Address+30,xData8,length);	// doc so luong it
////							Send2((uint16_t *)xData8,length);
////							return;
////						}
////						else				// neu length lon hon fileread
////						{
////							FLASH_ReadBuffer(Address+30,xData8,1024);		// doc 1024 byte 1 lan
////							Send2((uint16_t *)xData8,1024);
////							Delay_ms(2);
////							length=length-1024;
////							Address=Address+1024;
////							if(length<(1024-30))
////							{
////								FLASH_ReadBuffer(Address+30,xData8,length);		// doc 1024 byte 1 lan
////								Send2((uint16_t *)xData8,length);
////							}
////						} 
////						
////					}
//				}
//				else // Lon hon1 sector
//				{
//					FLASH_ReadBuffer(Address+30,xData8,1024);
//					Send2((uint16_t *)xData8,length);
//					Number++;
//				}
...
{
	uint8_t i,y,xfilename[1024],Data8[5],Copy=0;
	uint16_t Number,Add,Data16[2],Address;
	Sendstring("Erase!\r");
	
	FLASH_ReadBuffer8(0,xfilename,1024);
	Delay_ms(10);
	/// Tim kiem vi tri, so luong cua filename trong listfile
	for(i=0;i<30;++i)
	{
